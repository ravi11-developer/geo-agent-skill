...
SITES_JSON = os.path.join(PROJECT_ROOT, "eval", "sites", "real", "sites_2600.json")
CORPUS_DIR = os.path.join(PROJECT_ROOT, "eval", "sites", "real", "corpus_2600")


def run_capture(limit: int = 0, pages: int = 2, workers: int = 20):
    cmd = [
        sys.executable,
        os.path.join(PROJECT_ROOT, "eval", "runners", "capture_corpus.py"),
        "--sites", SITES_JSON,
        "--out", CORPUS_DIR,
        "--pages", str(pages),
        "--workers", str(workers),
...
def run_benchmark(limit: int = 0, workers: int = 32, agents: list[str] = None):
    cmd = [
        sys.executable,
        os.path.join(PROJECT_ROOT, "eval", "runners", "run_real_suite.py"),
        "--corpus", CORPUS_DIR,
        "--workers", str(workers),
    ]
    if limit > 0:
