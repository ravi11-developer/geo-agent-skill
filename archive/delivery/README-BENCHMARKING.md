...
## Appendix: the hybrid LLM layer and its ablation

The marketplace agent gained an **optional** hybrid LLM layer. It is `off` by
default, needs no API key, and with the layer off the agent emits exactly the
report it emitted before — verified byte-for-byte against frozen golden reports
in `eval/tests/golden/` and across all 83 captured real sites.

### Commands

```bash
python eval/tests/run_tests.py                     # 190 offline tests, fake provider only
python eval/runners/run_suite.py                   # the 16-site suite, unchanged
python eval/runners/run_ablation.py                # 5 feature modes, 16 synthetic sites
python eval/runners/run_ablation.py --corpus       # 5 feature modes, the captured corpus
python eval/agents/marketplace/tools/build_submission.py --zip   # package + validate + size check
```

### Modes

`off` (default) · `suggestions_only` · `semantic_shadow` · `semantic_enabled` ·
`full`. Enable with **both** variables:

```bash
export LLM_ENABLED=true LLM_MODE=semantic_shadow
```

`LLM_ENABLED=false` (or unset) pins the mode to `off`, so a stray `LLM_MODE` in a
shell can never start paid calls. `LLMConfig.for_benchmark()` — used by the
2,600- and 10,000-site runners — refuses a paid provider outright unless
`LLM_ALLOW_BENCHMARK_CALLS=true`.

### Reading an ablation table honestly

The default responder is `offline-analyst`: a deterministic scripted responder,
**not a model**. It exercises the whole pipeline — prompt assembly, transport,
validation, repair, promotion, verification, merging, telemetry — reproducibly
and at zero cost. An ablation run with it measures *system behaviour*: what the
gate lets through, what promotion costs, runtime, token footprint, backward
compatibility. It says nothing about a real model's judgement. Claims about
model quality require `--responder anthropic --allow-paid-calls`.

### Scoring caveat that matters

Semantic findings sit outside the benchmark's graded eight-category taxonomy, so
their precision is **not measured** by the existing gold labels. They are
declared `other`, which is where the scorer puts findings outside the graded
vocabulary — neither credited nor penalised. Filing them as `engagement`
instead would drop micro-averaged precision from 0.952 to 0.833 on the ablation
set, because the gold correctly records no navigation defect on those sites.
That is why promotion is off by default and `semantic_shadow` is the highest
mode recommended for graded runs.

No gold label and no scoring weight was changed.
