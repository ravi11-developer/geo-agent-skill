...
# Platform hosts where an organisation's identity can be independently checked.
# These are *platforms*, not target sites: matching on them stays domain-agnostic
# and works for any site under audit.
IDENTITY_PLATFORMS = (
    "linkedin.com", "crunchbase.com", "wikipedia.org", "wikidata.org", "github.com",
    "twitter.com", "x.com", "facebook.com", "instagram.com", "youtube.com", "g2.com",
    "capterra.com", "trustpilot.com", "glassdoor.com", "bloomberg.com", "sec.gov",
    "companieshouse.gov.uk", "zaubacorp.com", "mca.gov.in", "doi.org", "orcid.org",
    "scholar.google.com", "researchgate.net", "yelp.com", "tripadvisor.com",
    "play.google.com", "apps.apple.com", "producthunt.com", "medium.com",
)
CORROBORATION_SCHEMA_KEYS = ("sameas", "citation", "review", "aggregaterating", "award",
                             "founder", "parentorganization", "memberof", "publisher")
CITATION_TAGS = ("cite", "blockquote")
...
# ---------------------------------------------------------------------------
# Corroboration: can anything the site claims be checked against a third party?
# ---------------------------------------------------------------------------

def collect_corroboration(snapshot: SiteSnapshot) -> dict[str, Any]:
    """Count every independent-verification signal the site publishes."""
    same_as: list[str] = []
    platform_links: list[str] = []
    external_links = 0
    schema_keys: set[str] = set()
    citation_elements = 0
    host = urlparse(snapshot.entry_url).netloc.lower().lstrip("www.")

    for page in snapshot.ok_pages:
        for obj in page.jsonld_objects:
            for key, value in obj.items():
                if key.lower() in CORROBORATION_SCHEMA_KEYS:
                    schema_keys.add(key)
                    if key.lower() == "sameas":
                        same_as.extend(value if isinstance(value, list) else [str(value)])
        for link in page.links:
            ...
            if not netloc or netloc == host:
                continue
            external_links += 1
            if any(platform in netloc for platform in IDENTITY_PLATFORMS):
                platform_links.append(href)
        for tag in CITATION_TAGS:
            citation_elements += len(page.soup.find_all(tag))
        citation_elements += len(page.soup.find_all(attrs={"rel": re.compile(r"author|publisher|external|cite", re.I)}))
    ...
    return {
        ...
        "citation_elements": citation_elements,
        "signals": len(same_as) + len(platform_links) + len(schema_keys) + citation_elements,
    }


def check_corroboration(snapshot: SiteSnapshot, profile: dict[str, Any],
                        corroboration: dict[str, Any]) -> tuple[list[dict], list[dict]]:
    """Fires only when a site that *makes* claims offers no way to verify any of them.

    Deliberately narrow: a site with a single LinkedIn link in its footer, one
    `sameAs` entry, or any citation markup is not reported. Zero signals of any
    kind, on a site that names itself and states facts, is the case where an AI
    system has nothing but the site's own word to go on.
    """
    findings: list[dict] = []
    recs: list[dict] = []
    entry = snapshot.entry_page
    ...
    if corroboration["signals"] == 0 and profile.get("canonical") and entry.word_count >= 120:
        ...
        recs.append(recommendation(
            "Declare external profiles in `sameAs` as well as in the footer",
...
def run(context: dict[str, Any]) -> SkillResult:
    ...
    corroboration = collect_corroboration(snapshot)
    result.artifacts["corroboration"] = corroboration
    ...
    corr_findings, corr_recs = check_corroboration(snapshot, profile, corroboration)
    result.findings.extend(entity_findings + fresh_findings + corr_findings)
    result.recommendations.extend(entity_recs + fresh_recs + corr_recs)
