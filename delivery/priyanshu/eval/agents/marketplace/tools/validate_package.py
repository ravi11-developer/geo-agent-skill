...
def validate_package(root: str, rep: Report) -> dict:
    ...
    for entry in manifest.get("skills", []):
        ...
        entrypoint = os.path.join(skill_dir, entry.get("entrypoint", ""))
        if rep.check(os.path.exists(entrypoint), f"skill:{sid}",
            ...
            rep.check(entry.get("entrypoint", "").startswith("scripts/"), f"skill:{sid}",
        ...
        if os.path.exists(sj):
            try:
                ...
                rep.check(data.get("entrypoint") == entry.get("entrypoint"), f"skill:{sid}",
